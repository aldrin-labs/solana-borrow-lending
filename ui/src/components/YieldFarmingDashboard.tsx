...
export const YieldFarmingDashboard: FC = () => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="card">
        <h2 className="text-xl font-bold mb-6">Available Farming Pools</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left text-text-secondary">
                ...
                <th className="pb-4 font-medium">APR</th>
            ...
            <tbody>
              {farmingPools.map((pool) => (
                <tr key={pool.id} className="border-t border-gray-700">
                  ...
                  <td className="py-4 text-green-500">{pool.apr}</td>
                  <td className="py-4">{pool.leverage}</td>
                  <td className="py-4">
                    <button
      ...
      {connected && userFarmingPositions.length > 0 && (
        <div className="card">
          <h2 className="text-xl font-bold mb-6">Your Farming Positions</h2>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="text-left text-text-secondary">
                  <th className="pb-4 font-medium">Pool</th>
                  <th className="pb-4 font-medium">Deposited</th>
                  <th className="pb-4 font-medium">Leverage</th>
                  <th className="pb-4 font-medium">Position Value</th>
                  <th className="pb-4 font-medium">Rewards</th>
                  <th className="pb-4 font-medium">APR</th>
                  <th className="pb-4 font-medium">Actions</th>
                </tr>
              </thead>
              <tbody>
                {userFarmingPositions.map((position) => (
                  <tr key={position.id} className="border-t border-gray-700">
                    ...
                    <td className="py-4">{position.leverage}</td>
                    <td className="py-4">{position.value}</td>
                    <td className="py-4 text-green-500">{position.rewards}</td>
                    <td className="py-4 text-green-500">{position.apr}</td>
                    <td className="py-4">
                      <div className="flex space-x-2">
                        <button className="btn-primary text-sm py-1">Claim</button>
                        <button className="btn-secondary text-sm py-1">Close</button>
                      </div>
                    </td>
                  </tr>
                ))}
