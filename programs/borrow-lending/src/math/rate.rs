...
use crate::math::{common::*, decimal::Decimal};
use crate::prelude::*;
...
impl Rate {
    ...
    fn wad() -> U128 {
        U128::from(consts::WAD)
    }

    // OPTIMIZE: use const slice when fixed in BPF toolchain
    fn half_wad() -> U128 {
        U128::from(consts::HALF_WAD)
    }

    /// Create scaled decimal from percent value
    pub fn from_percent(percent: u8) -> Self {
        Self(U128::from(percent as u64 * consts::PERCENT_SCALER))
    }
    ...
    /// Calculates base^exp
    pub fn try_pow(&self, mut exp: u64) -> Result<Rate> {
        let mut base = *self;
...
impl TryFrom<Decimal> for Rate {
    type Error = Error;
    fn try_from(decimal: Decimal) -> Result<Self> {
